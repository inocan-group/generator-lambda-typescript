 ...
 */
function copyDirectory(ctx, dir) {
    return __awaiter(this, void 0, void 0, function* () {
        const fqDir = path_1.join(ctx.sourceRoot(), dir);
        const files = yield (yield globby(`${fqDir}/**/*`)).map((f) => f.replace(ctx.sourceRoot(), ""));
        files.forEach((f) => ctx.fs.copy(path_1.join(ctx.sourceRoot(), f), path_1.join(ctx.destinationPath(), f)));
    });
}
