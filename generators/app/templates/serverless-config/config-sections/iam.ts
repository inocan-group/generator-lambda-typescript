...
import {
  ...
  IServerlessIAMRole,
  serverlessConfigHasApiGatewayTracing,
  serverlessConfigHasLambdaTracing,
} from "common-types";
...
export function iamRoleStatements(config: IServerlessAccountInfo): { iamRoleStatements: IServerlessIAMRole[] } {
  ...
  return {
    ...
    ...(config.tracing
      ? {
          tracing: {
            apiGateway: serverlessConfigHasApiGatewayTracing(config),
            lambda: serverlessConfigHasLambdaTracing(config),
          },
        }
      : {}),
  };
