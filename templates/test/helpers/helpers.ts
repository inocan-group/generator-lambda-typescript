...
const stage = process.env.AWS_STAGE as (keyof IStagedVariables & string) | undefined;
export function setupEnv() {
  if (!envIsSetup) {
    if (!stage) {
      process.env.AWS_STAGE = "test";
    }
    const yamlConfig = yaml.safeLoad(fs.readFileSync("./env.yml", "utf8")) as IStagedVariables;
